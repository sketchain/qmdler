...
- **`vi[5] - vi[4]` 恒为 30000ms，`file.try_end - try_begin` 因曲而异，且不总是更大。**
  ...
  不存在主备关系，任何一个都可能缺席。
- **在一首歌整体可播的前提下，`size_* > 0` 与「取链能成功」完全一致。**
  对 8 首可播放的歌逐档探了 105 个 `size_* > 0` 的档位，**无一取链失败**；
  反过来，`size_*` 为 0 的档位（如没做过母带的曲目请求 `MASTER`）稳定返回 `104003`。

  **注意这个结论有前提。** 整首歌被地区版权分区挡住时，它的 `size_*` 照样是非零的，
  但所有档位一律 `104003`（「晴天」15 个档位全中）—— 所以 `size_*` 预测的是
