...
> ## ⚠️ 用途声明
>
> **本项目是作者的个人自用工具**，不是面向大众的通用下载器，也不追求对陌生环境的普适性。
>
> 仅供**个人学习**与**备份自有权益内容**使用。请勿用于任何形式的分发、传播或商业用途。
> 下载的内容版权归权利人所有，**不要再分发**。由使用者自行承担因使用本工具产生的一切责任。
>
> 使用前请读完 [已验证范围与未验证范围](#已验证范围与未验证范围)
> —— 这套东西只在很窄的一组条件下被验证过。
...
- **五种登录方式**：QQ 扫码 / 微信扫码 / QQ 音乐 APP 扫码（MQTT 长连接实时推送）/ 手机号短信 /
  直接导入 Credential（**只实测过 QQ 扫码**，见[已验证范围](#已验证范围与未验证范围)）
- **八种歌单来源**：自建歌单、收藏歌单、我喜欢、歌单 ID 或分享链接、专辑、歌手全部作品、搜索结果、手动 mid 列表
- **音质优先级链**：拖拽排序，逐档尝试，实际拿到的档位低于首选时明确标注「已降级」
- **四层试听检测**：`purl` 前缀 → `size_try` 绝对值比对 → Content-Length 比对 → 落盘后时长校验。
  检出的试听片段记为独立状态 `trial`，**绝不混进「成功」**；
  某一层因数据缺失没跑成时标 `skipped` / `unverified`，**绝不标 `ok`**
...
> **首次使用请先用 `qmdler single <songmid>` 跑一首。**
>
> 它会把四层校验每一层的实际判定值摊开给你看。先确认在**你自己的**平台、网络和账号
> 权益下四层都正常（尤其是取链 `result`、前缀比对、Content-Length 与 `size_*` 的差异），
> 再开始批量下载。这一步花不到一分钟，能省掉「下了三百首才发现全是试听片段」。
...
这一节存在的理由：这个项目反复踩到的失效模式是**「显示绿灯但代码根本没执行」**
——MP3 标签从未写入、`.nac` 上四层校验悄悄变三层、环境变量覆盖从未生效。
所以「验证过什么」必须和「没验证过什么」一样明确。

### 已实测验证（附验证方式）

| 声称 | 怎么验的 |
|---|---|
| 严格串行、每首一次取链 | 长跑 20 首，`get_song_urls` **20 次，比值 1.00 : 1** |
| 403 换节点、不重新取 vkey | 长跑后节点的连续拒绝计数最大值仍是 **1**（若按节点累积应是十几） |
| `.part` 断点续传 | `kill -9` 于 90 MB，新进程用**新 vkey** 续完 240 MB，与一次性下载**逐字节相同**；续传结果单独跑完 ffprobe / ffmpeg / `flac -t` / PCM-MD5 |
| 四层试听检测、trial 不混进 success | 全档位 13 档逐档跑，六条判定记录齐全 |
| 扩展名映射 | 13/13 与 `SongFileType.e` 相符（含 `ATMOS_DB`→`.mp4` 这类易错项） |
| 容器与 codec 自洽、字节数与 `size_*` 相等 | 13/13，差 0 |
| 四套 tag 写法 | 五种容器写 tag 前后 `ffmpeg` 全量解码 stderr 全空，封面与歌词读回确认 |
| 主动凭证刷新 | 原始链路 + `AuthManager.refresh()` 两层；`device.json` 前后 sha256 一致 |
| 分页翻完 | 5075 首的歌单拉到 5075/5075，零重复，51 次请求，峰值内存 26.6 MB |
| 分页限速 | 请求间隔中位 1.53 s，**低于 0.4 s 的 0 次** |
| 拉取可中断 | 中途取消：抛异常、**不返回半截列表**、立刻重拉不受污染 |
| 目录可写探针 / 磁盘空间 / 目录自动创建 | `tests/integration/test_claims_audit.py` |
| 孤儿 `.part` 清理 | 同上（孤儿删、进行中留、正式文件留） |
| 环境变量覆盖 / 预设切换 | 同上 + 真起服务验证 |
| 两端状态一致 | 同上（取消广播、设置变更广播） |
| 大列表可用性 | WebUI 5075 行首屏 74 ms、DOM 26 行；TUI 填表 0.93 s |
| WebUI 全流程 | 真开浏览器只点界面走通「设置 → 选来源 → 建任务 → 下载 → 报告」，无 CLI 专属路径 |

### 未验证，使用者需自行注意

- **仅在 Linux 下验证过。Windows / macOS 从未运行。**
  文件名清洗（保留名、按字节截断）、路径长度上限、`.part` 的同目录 rename
  都是平台相关行为。其他平台首次使用请**先小批量试跑**。
- **仅用单一超级会员（SVIP）账号验证过。**
  非会员账号会大量走 `104003` 与降级路径，而**降级链记账
  （`degraded` / `requested_quality` / `actual_quality`）是本项目唯一从未被真实触发的
  核心路径** —— 实测 8 首歌共 105 个 `size_* > 0` 的档位无一取链失败，
  构造不出自然场景，只有 mock 覆盖。
- **仅在单一网络环境下验证过。** 该出口 IP 存在地区版权分区限制，部分曲目全档 `104003`。
  换环境后行为会不同。`tests/fixtures/` 里快照的采集条件已在 `tests/fixtures/README.md` 注明
  —— 在别的网络下重抓时 `result` 会变，**那不是回归**。
- **最长连续运行 20 首 / 约 1 小时。**
  数千首级别的歌单需要连续跑几天，期间**必然跨越凭证 72 小时有效期**。
  刷新链路已单独验证，但**从未在长跑中途真实触发过**。
- **风控暂停（`RatelimitedError`）仅有 mock 覆盖**，没有真去触发风控。
- **登录方式只验证过 QQ 扫码。**
  微信扫码、QQ 音乐 APP 扫码（MQTT）、手机验证码、导入 Credential 均未实测。
- **`sa` 字段不参与任何判断。** 它的位布局在库源码与官方文档里都没有定义，
  详见[与上游库的已知差异](#与上游库的已知差异)。
...
2. **`size_try` 绝对值比对** —— 该值是「试听片段有多大」的基准，**不是本曲正片的特征值**。
   完整表述是：**每曲一值、与档位无关、跨曲取值离散**。同一首歌的 13 个档位拿到的
   `size_try` 全部相同（都是 `960887`）；跨 12 首歌则只出现三个取值：`960887`
   （8 首，≈128kbps × 60s）、`481070`（1 首，约一半）、`0`（3 首，该曲没有这项）。
   （这条表述改过两次：先说「跨曲恒定」被扩样本推翻，再补上「同曲各档相同」才完整。）

   所以拿它跟正片大小做「交叉验证」量纲都不对；正确用法是文件大小落在 `size_try` ±2%
   内直接判 `trial` —— 与 `interval` 无关、与档位无关。始终读响应里的字段，
   **不要硬编码 `960887`**；`size_try == 0` 时本层放行
...
- **大歌单实测（5075 首的收藏歌单，只拉列表不下载）**：拉到 5075/5075、零重复、
  零缺 `media_mid`，51 次请求、79s、`tracemalloc` 峰值 26.6 MB。
  **翻页之间必须限速** —— 库里的 `paginate()` 是背靠背连打的，51 次连发是很明显的
  风控特征，所以页与页之间插了与元数据同一套的轻量延迟（实测请求间隔最小 1.29s、
  中位 1.53s，低于 0.4s 的间隔 0 次）。翻页同时支持取消（不返回半截列表）与进度推送。
- **WebUI 在 5075 行下必须窗口化。** 全量渲染实测首屏 **4112ms**、清空过滤 **3165ms**，
  而且表格与卡片两套布局是**同时**挂在 DOM 里靠 CSS 藏掉一套的（10150 个节点）。
  改成「只渲染当前布局 + 只渲染视口内的行」之后：首屏 **74ms**、清空过滤 **38ms**、
  全选 **44ms**，DOM 行数 10150 → 26。行高**在运行时量**而不是写死 ——
  实测按 44px 算、真实渲染 61px，写死会让偏移越滚越歪。
  TUI 不用改：Textual 的 `DataTable` 自己就是虚拟化的（5075 行填表 0.93s、方向键 0.17s）。
...
- [LICENSE](LICENSE) 是 GPL-3.0 全文；
- `pyproject.toml` 的 `license` 字段与 classifier 均为 GPL-3.0-or-later；
- 所有自有源文件（`src/` `scripts/` `tests/` 下的 `.py` / `.js` / `.css`）头部都带
  `SPDX-License-Identifier: GPL-3.0-or-later`。

**这不是可选项**：核心依赖 [`qqmusic-api-python`](https://github.com/L-1124/QQMusicApi)
是 GPL-3.0，本项目链接它，因此必须同样以 GPL-3.0 发布。

`src/qmdler/web/vendor/` 下是**第三方原样分发**的文件，各自遵循自己的许可证，
未被添加任何头部：

| 文件 | 项目 | 许可证 |
|---|---|---|
| `vue.global.prod.js` | Vue 3 | MIT |
| `sortable.min.js` | SortableJS | MIT |

再次强调：本工具仅供个人学习与备份自有权益内容使用，**不要再分发所下载的内容**。
